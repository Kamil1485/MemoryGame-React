...
const cardImages = [
  ...
  { src: "/img/a1.png",name: "a1", matched: false },
  { src: "/img/b1.png",name: "b1", matched: false },
  { src: "/img/t.png",name: "b1", matched: false },
  { src: "/img/w.png",name: "b1", matched: false },
];
function App() {
  ...
  const [cardPair, setCardPair] = useState(); //Kullanıcı cart çifti sayısı belirleyebilmesi için;
  ...
  const shuffleCards = () => {
    ...
    const shuffledCards2 = [...cardImages].sort(() => Math.random() - 0.5);
...
*/
  ...
  useEffect(() => {
    ...
    if (choiceOne && choiceTwo) {
      //2secimde yapıldıysa karşılaştırmaya başla
      //19-
        ...
        setCards((prevCards) => {
          return prevCards.map((cardItem) => {
            ...
            if (cardItem.name === choiceOne.name) {
              ...
              if (gameCount === cardPair.length / 2) {
  ...
  return (
    <div className="App">
      {gameOver ? (
        <div>{<GameOver turns={turns} shuffleCards={shuffleCards} setGameOver={setGameOver} restartGame={restartGame}/>}</div>
      ) : (
        <div>
          <h1>Hafıza Oyunu</h1>
          <div className="cardPair">
            ...
            <input
              ...
              min="2"
              max="10"
              placeholder={`2-${cardImages.length} sayı giriniz`}
              value={cardPair}
              onChange={(e) => setCardPair(e.target.value)}
            />
            <button
              className="btn"
...
//FİLET METODU VE OPTİONS KISMI İÇİN BUU APP3 KOMPLE